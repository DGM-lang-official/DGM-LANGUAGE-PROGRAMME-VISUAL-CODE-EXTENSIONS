...
fn regex_match(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    match (a.get(0), a.get(1)) {
        (Some(DgmValue::Str(text)), Some(DgmValue::Str(pat))) => {
            let re =
                regex::Regex::new(pat).map_err(|e| DgmError::runtime(format!("regex: {}", e)))?;
            Ok(re
                .find(text)
                .map(|m| DgmValue::Str(m.as_str().to_string()))
                .unwrap_or(DgmValue::Null))
        }
        _ => Err(DgmError::runtime("regex.match(text, pattern) required")),
    }
}
...
fn regex_find_all(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    match (a.get(0), a.get(1)) {
        (Some(DgmValue::Str(text)), Some(DgmValue::Str(pat))) => {
            let re =
                regex::Regex::new(pat).map_err(|e| DgmError::runtime(format!("regex: {}", e)))?;
            let matches: Vec<DgmValue> = re
                .find_iter(text)
                .map(|m| DgmValue::Str(m.as_str().to_string()))
                .collect();
            Ok(DgmValue::List(Rc::new(RefCell::new(matches))))
        }
        _ => Err(DgmError::runtime("regex.find_all(text, pattern) required")),
    }
}
...
fn regex_replace(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    match (a.get(0), a.get(1), a.get(2)) {
        ...
        _ => Err(DgmError::runtime(
            "regex.replace(text, pattern, replacement) required",
        )),
    }
}
...
fn regex_test(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    match (a.get(0), a.get(1)) {
        ...
        _ => Err(DgmError::runtime("regex.test(text, pattern) required")),
    }
}
...
fn regex_split(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    match (a.get(0), a.get(1)) {
        (Some(DgmValue::Str(text)), Some(DgmValue::Str(pat))) => {
            let re =
                regex::Regex::new(pat).map_err(|e| DgmError::runtime(format!("regex: {}", e)))?;
            let parts: Vec<DgmValue> = re
                .split(text)
                .map(|s| DgmValue::Str(s.to_string()))
                .collect();
            Ok(DgmValue::List(Rc::new(RefCell::new(parts))))
        }
        _ => Err(DgmError::runtime("regex.split(text, pattern) required")),
    }
}
